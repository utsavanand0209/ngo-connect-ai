...
const getAdminDashboardSnapshot = async ({ limit, days, noCache = false }) => {
  const key = `${limit}:${days}`;
  if (!noCache) {
    const cached = dashboardSnapshotCache.get(key);
    if (cached && Date.now() - cached.timestamp < DASHBOARD_SNAPSHOT_CACHE_TTL_MS) {
      return cached.value;
    }
  }

  const snapshot = await computeAdminDashboardSnapshot({ limit, days });
  if (!noCache) {
    dashboardSnapshotCache.set(key, { timestamp: Date.now(), value: snapshot });
  }
  return snapshot;
};
...
router.get('/dashboard', auth(['admin']), async (req, res) => {
  try {
    ...
    const noCache = ['1', 'true', 'yes'].includes(String(req.query.noCache || '').trim().toLowerCase());
    const snapshot = await getAdminDashboardSnapshot({ limit, days, noCache });
    res.json(snapshot);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
...
// Server-side rendered HTML snapshot for the admin dashboard.
// This is intended to be opened from the SPA (XHR + window.open) so client-side JS can refresh using the stored JWT.
router.get('/dashboard/ssr', auth(['admin']), async (req, res) => {
  try {
    ...
    res.send(renderAdminDashboardHtml(snapshot));
  } catch (err) {
    console.error(err);
